    ...
    for document_embedding in document_embeddings:
        ...
        for candidate in document_embedding['candidates']:
            candidates_document_similarites.append(
                cosine_similarity(candidate.reshape(1,-1), text_embedding.reshape(1,-1))[0]
            )
    ...
    for document_embedding in document_embeddings:
        ...
        for candidate1 in document_embedding['candidates']:
            ...
            for candidate2 in document_embedding['candidates']:
                candidates2.append(
                    cosine_similarity(candidate1.reshape(1, -1), candidate2.reshape(1, -1))[0]
                )
...
def calculate_and_write_pickle_cossim(labeled_documents_path, embeddings_path,cosine_similarities_path):
    documents_json = open(labeled_documents_path)
    documents = json.load(documents_json)
    ...
    for index, document in enumerate(documents):
        document['sim_candidates_candidates'] = cosine_values_can_can[index]
